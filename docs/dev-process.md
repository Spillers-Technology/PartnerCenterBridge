...
- **Sol/high** — architecture, security, tricky debugging, anything touching auth, RBAC,
  tenant isolation, or a security-relevant state machine (staging/approval/token flows), and
  thread/lifetime/shutdown/resource-release work regardless of diff size. (Corrected 2026-08-24:
  the original row was copied verbatim from a sibling project's concurrent control-loop context
  and omitted the risks this repo has actually escalated. Every Sol/high escalation logged below
  so far (Units 1, 5, 6, 8, 13, 14) is auth/RBAC/tenant-isolation/security-state-machine work.
  Resource-lifetime risk still exists here in process, stream, cancellation, and concurrent
  approval paths, so it remains an escalation trigger when encountered; the correction adds the
  observed domain rather than pretending the original trigger disappeared.)
...
### Third tooling bug — `[windows] sandbox = "elevated"` broke Codex's shell tool machine-wide

Unit 14's first whole-branch Sol/high dispatch had its shell tool fail immediately with
`CreateProcessWithLogonW failed: 2`, then silently fell back to GitHub's MCP tools and web search
to read files instead of local disk -- its closing line disclosed this, which is the only reason it
was caught (same class of near-miss as the Second tooling bug above). The identical error appears
in Unit 13's row below, but was treated there as a one-off Windows process-runner quirk worked
around by running commands independently -- it recurred on Unit 14's very next Codex dispatch,
which is what prompted actually finding the root cause this time instead of working around it
again. Reproduced directly: a
throwaway unsandboxed `codex exec ... "git log -1"` call on the same machine failed identically, so
this wasn't a subagent-sandbox artifact -- it was `~/.codex/config.toml`'s `[windows] sandbox =
"elevated"` setting, which spawns Codex's sandboxed subprocess via `CreateProcessWithLogonW`, itself
broken at the OS layer. Fixed by changing that setting to `"unelevated"`, confirmed immediately with
a throwaway `codex exec -m gpt-5.6-luna -s read-only "git log -1"` ping that returned real local
output before trusting any real dispatch again.

**Rule: when a Codex dispatch's own report mentions any shell/exec failure, verify the fix works
with a cheap throwaway ping before re-dispatching the real task** -- the same "verify before
trusting" discipline the Second tooling bug established, now confirmed to also cover machine-level
sandbox config, not just per-repo or per-diff-size failures. Since this setting is global
(`~/.codex/config.toml`, not per-project), it silently would have broken every Codex dispatch across
every repo on this machine, not just this one -- worth an occasional throwaway ping at the start of
a session if Codex hasn't been used in a while.
...
| 13 | Contracts desired-app editor: model correction, admin-gated mutations, inline concurrent checklist, quest-chip package repair | Claude/controller handoff -> controller fix waves | Sol/high independent review + Codex CLI Sol/high, two scoped re-reviews | 25 review claims (19 unique real defects) | 19 | 0 | ~173,000 Codex CLI review tokens, plus the independent pass | The review layer overturned the approved spec's central premise: `Contract.DesiredApps` was actually a one-to-many `AppTemplate.ContractId`, so assigning a reusable template to contract B silently removed it from A. The fix added a data-preserving many-to-many migration and a two-contract persistence test. Review also found missing server authorization on package upload/create, a server-side package-readiness bypass, whole-DTO and full-list/plan stale-response races, ambiguous post-commit failure handling, overlapping uploads, a keyboard-inaccessible quest chip, hidden template-load failures, misleading legacy package-less state, and non-inline placement. First re-review caught five self-inflicted/residual gaps (package-less create and removal, late plan invalidation, lost just-created rows, false upload failures) plus concurrent HTTP idempotence; all were reproduced and fixed. Final scoped Sol/high re-review returned CLEAN. Codex's own Windows process runner failed with `CreateProcessWithLogonW failed: 2`, so its test grades were static; the controller independently ran every targeted and full command outside that runner, following the standing verify-before-trusting rule. |
| 14 | Delegated instance RBAC (fixed roles: Administrator, CatalogManager, CredentialManager, AutomationPolicyManager) layered alongside the pre-existing per-tenant Viewer/Operator/Owner plane; authorization sweep across every REST/MCP surface; data-preserving migration; Postgres locking tests | Handoff from a prior session (implementer unrecorded) -> controller fix waves | Codex CLI Sol/high whole-branch review, then a second scoped Sol/high re-review | 4 (whole-branch) + 3 (scoped re-review) | 4 + 3 | 0 | ~157,900 (whole-branch) + ~25,000 (failed re-review attempts) + ~5,000 (successful scoped re-review) | The whole-branch review's first dispatch hit the Third tooling bug below and silently reviewed remote `main` instead of local `feat/delegated-rbac` for context around the diff -- caught by the closing disclosure, and independently confirmed harmless in this instance because its two most-cited files were pre-existing on `main`, not this branch's diff. Findings: 1 High (confirmed, pre-existing, not this branch's regression) -- `ConfigSnapshotsController.BuildDiffsAsync` resolved `beforeRunId`/`afterRunId` with no tenant filter despite the action authorizing only the route's own tenantId, letting a Viewer of one tenant pull another tenant's Conditional Access/device-compliance snapshot diff by guessing a run ID; 1 Medium (confirmed, also pre-existing) -- `PasskeyController.LoginVerify` and `TotpController.Challenge` issued a full `AuthResponse` for a deactivated account, unlike `AuthController`'s existing `IsActive` check (contained by `McpTokenValidator` rejecting the resulting token on the next protected call, but still a real inconsistency and a `MeDto` leak); 1 Medium assurance gap -- the branch's own new `TenantAuthorizationSweepTests.cs` covers dashboard filtering, directory/exchange/provisioning forbids, and last-owner protection but not config snapshots, deployments, search, workflows, or contracts, and specifically missed the confirmed disclosure; 1 Low -- the migration's same-commit column drop isn't rolling-deployment safe (acceptable for this app's pinned single-replica deployment). Both pre-existing bugs were confirmed by the controller reading the actual code directly (not just trusting the review, which had partially worked from a stale basis) before fixing, per the user's explicit choice to fix both now rather than defer. Fixed with the same tenant/IsActive scoping pattern already used elsewhere in the codebase, plus 2 new test files. The scoped re-review of that fix commit came back "clean to merge" with 3 Low findings, all real: the config-snapshot test only proved a foreign `afterRunId` is rejected, not `beforeRunId` independently; the TOTP test's deliberately-invalid protected secret meant it would have failed the pre-fix code for the wrong reason (an `Unprotect` exception, not the actual bug); and Passkey's `LoginVerify` fix had no dedicated test at all, reasoned as disproportionate effort to avoid mocking Fido2's cryptographic assertion verification -- the review correctly pointed out the `IsActive` guard runs *before* `MakeAssertionAsync` is ever called, so only a fake `IFido2` that throws if reached is needed, no crypto mocking required. All three fixed and, per this log's own standing rule, verified directly: reverted to the pre-fix controllers and confirmed each test actually failed against that code before restoring the fix. Confirms the standing rule with a new wrinkle: a review's own infrastructure failure doesn't necessarily invalidate its findings if the affected files happen to be outside the diff being reviewed, but it must still be caught and disclosed rather than assumed harmless in general. |
