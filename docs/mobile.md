...
Register, **Workflows**, and **Approvals**. The last two are newly migrated (Operate group,
workstream 2 of the 0.6.0 MUI migration) -- both were in the original pre-migration overflow
baseline and now pass the full 5-device matrix cleanly (`PCBRIDGE_CAPTURE_VIEWS=workflows,approvals`,
zero-overflow exit code), confirmed directly, not assumed.
...
Contracts, App Templates, History, Config Snapshots, and Security.
